...
public class RequestToUserUtil {
    ...
    public User resetUser(HttpServletRequest request) {
        UserDetails userDetails = userService.loadUserByMobile(request.getParameter("mobile"));
        if (!(userDetails instanceof User)) {
            LOG.error("userDetails类型转化为user时发生错误");
            throw new RuntimeException();
        } else {
            User.Builder builder = new User.Builder((User) userDetails)
